...
#MIT_DIR = "/Users/szelie/Library/CloudStorage/OneDrive-ETHZurich/data/MIT_windfields/windfields"

MIT_DIR = "/nfs/n2o/wcr/szelie/MIT_windfields/windfields"
...
def load_tc_realization(path, year_start, year_end, real_id=0, basin=None, draws_subset=None):
    ...
    #draws = draws[(draws['real_id'] > n_realisations+25)&(draws['real_id'] <= n_realisations+50)].reset_index(drop=True)
    ...
    for warming_level in ['1', '2']:
        ...
        for r, rcp in enumerate(RCP):
            for g, gcm in enumerate(GCM):
                gcm = gcm.lower()
                if years_warming_level[warming_level][rcps[r]][gcm] is None:
                    continue
                year_start, year_end = years_warming_level[warming_level][rcps[r]][gcm]
                draws_subset = np.random.choice(np.arange(1, 100), 25)
                for basin in basins:
                    ...
                    event_name = [f"{str(datetime.date.fromordinal(date).year)}_{gcm.lower()}_{rcps[r]}" for date in haz.date]
        ...
        hazard_path = f"/nfs/n2o/wcr/szelie/MIT_windfields/hazard_TC_MIT/hazard_tc_{warming_level}.hdf5"
